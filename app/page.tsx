    ...
    <div className="vignette">
      <header className="mx-auto max-w-6xl px-5 pt-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold tracking-tight">AfterCredits</span>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-5 pb-20">
        <section className="mt-8">
          <div className="marquee-frame p-4">
            <div className="flex justify-center gap-2 px-4 pb-3">
              {Array.from({ length: 42 }).map((_, i) => <span key={i} className="bulb opacity-95" />)}
            </div>
            ...
            <div className="marquee-inner px-8 py-10 text-center">
              ...
              <p className="mt-4 text-sm sm:text-base text-zinc-800/80">
                AfterCredits captures the audience reaction and extracts the signal using AI.
              </p>
            ...
            <div className="flex justify-center gap-2 px-4 pt-3">
              {Array.from({ length: 42 }).map((_, i) => <span key={i} className="bulb opacity-90" />)}
            </div>
          </div>
        </section>

        <section id="how" className="mt-16 text-center">
          <h2 className="text-3xl sm:text-4xl font-semibold" style={{ fontFamily: "var(--font-playfair)" }}>
            How it works
          </h2>

          <div className="mt-10 grid gap-8 sm:grid-cols-3 text-left">
            <Step num="01" title="Set the screening" body="Enter the movie title for the session." />
            <Step num="02" title="Collect reactions" body="Share quick thoughts and ratings." />
            <Step num="03" title="Extract the signal" body="AI summarizes the feedback." />
          </div>
        </section>
        ...
        <section id="preview" className="mt-16 text-center">
          ...
          <p className="mt-2 text-white/80">What you&apos;ll see on the board.</p>

          <div className="mt-10 grid gap-6 lg:grid-cols-2 items-start">
            <div className="marquee-frame p-4">
              <div className="flex justify-center gap-2 px-4 pb-3">
                {Array.from({ length: 24 }).map((_, i) => <span key={i} className="bulb opacity-95" />)}
              </div>

              <div className="marquee-inner p-6 text-left">
                <p className="text-sm font-semibold">Audience Summary</p>
                <div className="mt-5 flex items-center justify-between gap-4 text-sm">
                  <span className="text-zinc-800/80">Tonight&apos;s Screening</span>
                ...
                <ul className="mt-5 space-y-2 text-sm text-zinc-800/80">
                  <li className="flex items-center gap-2">
                    ...
                    Most praised: <span className="font-medium text-zinc-900">Cinematography</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="h-2 w-2 rounded-full bg-emerald-600" />
                    Most debated: <span className="font-medium text-zinc-900">Ending</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="h-2 w-2 rounded-full bg-emerald-600" />
                    Overall tone: <span className="font-medium text-zinc-900">Positive</span>
                  </li>
                </ul>

                <div className="mt-5 text-sm text-zinc-800/80">
                  <span className="font-semibold text-zinc-900">AI summary:</span>{" "}
                  Visually stunning, divisive ending
                </div>
              </div>

              <div className="flex justify-center gap-2 px-4 pt-3">
                {Array.from({ length: 24 }).map((_, i) => <span key={i} className="bulb opacity-90" />)}
              </div>
            </div>

            <div className="space-y-4">
              <Review name="Mia" stars={5} text="The cinematography was unreal. Ending had me thinking for hours." />
              <Review name="Tim" stars={4} text="Loved the score, but the pacing was a bit slow." />
              <Review name="Alex" stars={4} text="Cool concept, but the characters felt flat." />
            </div>
          </div>
        </section>

        <section className="mt-20 text-center">
          <h3 className="text-3xl sm:text-4xl font-semibold" style={{ fontFamily: "var(--font-playfair)" }}>
            Start your next movie night board.
          </h3>
          <p className="mt-3 text-white/80">
            Set the title, share the link, and capture the room&apos;s reaction in real-time.
          </p>
